...
import init, { MZFMachine, process_binary } from './pkg/rust_wasm_converter.js';
...
const modeZX81Basic = document.getElementById('modeZX81Basic');
...
if (modeParam) {
    ...
    else if (modeParam === 'ZX81BASIC') modeZX81Basic.checked = true;
    else modeDump.checked = true; // Default to DUMP if invalid mode
} else {
    // This is a placeholder for `script.js`. The actual default will be set in the specific HTML files.
...
init().then(() => {
    ...
    processFile = () => {
        ...
        else if (modeZX81Basic && modeZX81Basic.checked) mode = 'ZX81BASIC';
        ...
        try {
            ...
            const escapeHtml = (text) => text.replace(HTML_ESCAPE_REGEX, (match) => HTML_ESCAPE_MAP[match]);
            ...
            while (i < result.length) {
                ...
                if (char === '§') {
                    ...
                    if (endIndex === -1) {
                        // No closing § found, treat rest as inverse
                        const content = escapeHtml(result.substring(startIndex));
                        htmlParts.push(`<span class="inv-char">${content}</span>`);
                        break;
                    } else {
                        const content = escapeHtml(result.substring(startIndex, endIndex));
                        htmlParts.push(`<span class="inv-char">${content}</span>`);
                        i = endIndex + 1;
                    }
                } else {
                    ...
                    if (i > normalStart) {
                        const normalText = escapeHtml(result.substring(normalStart, i));
                        htmlParts.push(normalText);
                    }
                }
            }

            outputPre.innerHTML = htmlParts.join('');

            // outputPre has been updated
        } catch (e) {
